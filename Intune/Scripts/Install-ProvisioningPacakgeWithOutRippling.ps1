...
.DESCRIPTION
    ...
    A pre-flight check inspects the MDM enrolment registry keys under
    HKLM:\SOFTWARE\Microsoft\Enrollments. If the device is still managed by
    Rippling MDM (detected via ProviderId or UPN), the script exits gracefully
    without installing the provisioning package.
...
# ---------------------------------------------------------------------------
# Pre-flight check – abort if the device is still managed by Rippling MDM
# ---------------------------------------------------------------------------
...
if (Test-Path -Path $mdmEnrollmentPath) {
    ...
    foreach ($key in $enrollmentKeys) {
        ...
        $providerId = (Get-ItemProperty -Path $key.PSPath -Name "ProviderId" -ErrorAction SilentlyContinue).ProviderId

        if ($upn -or $providerId) {
            # Rippling MDM enrolments typically carry the provider id "RipplingMDM"
            # or a UPN / discovery endpoint that contains "rippling".
            if ($providerId -match "rippling" -or $upn -match "rippling") {
                Write-Output "Device is still managed by Rippling MDM (ProviderId: $providerId, UPN: $upn). Exiting."
                exit 2
            }
        }
    }
...
Write-Output "No Rippling MDM enrolment detected. Proceeding with provisioning package installation."
